...
import type { Characteristic, Measure, Product } from '@/pages/product/types';
...
const addCharacteristic = () => {
  if (data.value) {
    data.value.characteristics.push({
      characteristic: {
        id: 0,
        name: '',
        description: ''
...
<template>
  <main v-if="data">
    ...
    <section :class="[{ error: errors.error }, 'card']">
      ...
      <ComponentDropdown label="Категория" :value="category.categories" v-model="data.category" k="category"></ComponentDropdown>
    </section>

    <section class="card" v-for="(c, i) of data.characteristics" :key="i">
      <ComponentDropdown label="Характеристика" :value="characteristic.characteristics" v-model="data.characteristics[i].characteristic" k="characteristic"></ComponentDropdown>

      <ComponentDropdown label="Ед. измерения" :value="measure.measures" v-model="data.characteristics[i].measure" k="measure"></ComponentDropdown>

      <ComponentInput label="Значение" v-model="data.characteristics[i].value" type="text" k="value"></ComponentInput>
    </section>
